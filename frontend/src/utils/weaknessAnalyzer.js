/**
 * Weakness Analyzer Utility for Flow
...
export function analyzeWeaknesses(keystrokes = [], targetText = '') {
  ...
  for (let i = 0; i < keystrokes.length; i++) {
    ...
    const latency = Math.min(Math.max(ks.latency || 0, 0), 4000); // cap extreme pauses
  ...
  // Calculate baseline latency (median of valid correct keystrokes, default 200ms)
  ...
  // 2. Bigram analysis (adjacent 2-char combinations)
  ...
  // 3. Trigram analysis (adjacent 3-char combinations)
  for (let i = 2; i < keystrokes.length && i < normalizedTarget.length; i++) {
    const trigram = normalizedTarget.slice(i - 2, i + 1);
    // Only focus on letter/punct trigrams without excessive spaces
  ...
  // Helper to compute weakness score
  const scoreToken = (item) => {
    ...
    let score = 0;
    // Error component (heavily weighted)
    if (item.errors > 0) {
      score += item.errors * 250;
      score += item.errorRate * 350;
    }

    // Latency component (penalize slow key transitions compared to baseline)
  ...
  // Combine candidates: filter for meaningful items with errors or high latency
  const allCandidates = [...scoredChars, ...scoredBigrams, ...scoredTrigrams].filter((item) => {
    // Exclude whitespace-only tokens
    if (item.token.trim() === '') return false;

    // Must have at least 1 error OR significantly slow latency (35%+ slower than baseline with at least 2 attempts)
  ...
  // Pick top 3-5 diverse weaknesses
  const selected = [];
